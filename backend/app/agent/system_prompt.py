"""
System Prompt Builder — assembles the agent's system prompt at runtime
with seller-specific and customer-specific data injected.
"""


# This is the full content from agent instruction.md, with template variables
...
- Create and confirm orders where the total value is under ₹{auto_approve_limit}
...
- Order value exceeds ₹{auto_approve_limit}
...
- The customer's ID for tool calls is: `{customer_id}`
- The seller's ID for tool calls is: `{seller_id}`
"""


class SystemPromptBuilder:
    """Constructs the complete system prompt with seller + customer context injected."""
    ...
    def build(
        ...
        seller_instruction: str | None = None,
    ) -> str:
        prompt = AGENT_INSTRUCTION_TEMPLATE.format(
        ...
        # Gap #3: When resuming from escalation, inject seller's instruction
        if seller_instruction:
            prompt += f"\n\n## OWNER'S INSTRUCTION (follow this)\nThe business owner has reviewed the escalation and provided this instruction. Follow it to continue the conversation:\n{seller_instruction}\n"
